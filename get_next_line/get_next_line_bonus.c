...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/31 00:10:02 by adahab            #+#    #+#             */
/*   Updated: 2024/12/31 00:14:00 by adahab           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	savedline[fd] = read_line(fd, savedline[fd]);
	if (!savedline[fd] || !*savedline[fd])
		return (free(savedline[fd]), savedline[fd] = NULL, NULL);
	output = extract_line(savedline[fd]);
	savedline[fd] = move_to_next_line(savedline[fd]);
	return (output);
}
